        ...
        while True:
            if current_status == WebsocketStatus.plan.value:
                if user_modification_request is None:
                    ...
                    cr_plan_obj = CRPlanRequest(**cr_plan_req)
                    ...
                    genai_model_raw = cr_plan_req.get("genai_model", "").lower()
                    if genai_model_raw.startswith("ollama"):
                        cr_plan_obj.genai_model = genai_model_raw
                    else:
                        gai_model = genai_model_raw.split("-")
                        ...
                        genai_model_variant = genai_models.get(gai_model[1], None)
                        if not genai_model_variant:
                            await ws_manager.send_json_obj(
                                CRPlanResponse(
                                    task_status=Status.failed.value,
                                    error=f"Unknown model variant: {gai_model[1]}"
                                ).json(), websocket)
                            await ws_manager.disconnect_and_close(websocket)
                            break
                    ...
                    #cr_plan_obj.genai_model = "gemini-1.5-flash-8b"

                    #cr_plan_obj.genai_model = genai_model_variant

                    threshold_task = start_thresholding.apply_async(
                        args=[cr_plan_obj.dict(), user_instructions]
                    )
                    ...
                    planning_task = start_planning.apply_async(
                        args=[cr_plan_obj.dict(), user_instructions, req_threshold]
                    )
                ...
                report_gen_task = start_generating.apply_async(
                    args=[cr_plan_obj.dict(), user_instructions, generated_plan]
                )
